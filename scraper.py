...
def roc_date_to_datetime(roc_str):
    """Convert ROC date string like '115/03/24' to datetime"""
    try:
...
def merge_and_dedupe(old_records, new_records):
    """Merge old and new records, deduplicate by (code, date, subject)"""
    seen = set()
    merged = []
...
def filter_last_n_months(records, months=3):
    """Keep only records within the last N months"""
...
def main():
    ...
    # If near start of year, also fetch previous year
    now = datetime.now()
    if now.month <= 3:

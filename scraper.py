import sys
import json
import time
import requests
import urllib3
from datetime import datetime
from bs4 import BeautifulSoup

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

print("Python:", sys.version)
print("Starting...")

BASE_URL = "https://mops.twse.com.tw"
KEYWORD = "\u7121\u64d4\u4fdd\u8f49\u63db\u516c\u53f8\u50b5"

def get_roc_year():
    return datetime.now().year - 1911

def search_market(market_type, year):
    # KIND: L=上市, O=上櫃
    kind = "L" if market_type == "sii" else "O"
    market_label = "\u4e0a\u5e02" if market_type == "sii" else "\u4e0a\u6ac3"
    print("Searching:", market_label, "year:", year)

    # Step 1: call new API to get redirect URL with encrypted parameters
    api_url = BASE_URL + "/mops/api/redirectToOld"
    payload = {
        "apiName": "ajax_t51sb10",
        "parameters": {
            "r1": "1",
            "keyWord": KEYWORD,
            "keyWord2": "",
            "year": str(year),
            "Orderby": "1",
            "KIND": kind,
            "CODE": ""
        }
    }
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Content-Type": "application/json",
        "Accept": "application/json, text/plain, */*",
        "Referer": BASE_URL + "/mops/",
        "Origin": BASE_URL,
    }

    try:
        resp = requests.post(api_url, json=payload, headers=headers, timeout=30, verify=False)
        resp.encoding = "utf-8"
        print("  Step1 HTTP:", resp.status_code)
        data = resp.json()
        print("  Step1 message:", data.get("message", ""))
    except Exception as e:
        print("  Step1 ERROR:", e)
        return []

    if data.get("code") != 200:
        print("  Step1 failed:", data)
        return []

    old_url = data.get("result", {}).get("url", "")
    if not old_url:
        print("  No redirect URL found")
        return []

    print("  Got redirect URL, fetching results...")

    # Step 2: fetch the actual results from the old URL
    try:
        resp2 = requests.get(old_url, headers={
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
            "Referer": BASE_URL + "/mops/",
        }, timeout=30, verify=False)
        resp2.encoding = "utf-8"
        print("  Step2 HTTP:", resp2.status_code, "length:", len(resp2.text))
    except Exception as e:
        print("  Step2 ERROR:", e)
        return []

    soup = BeautifulSoup(resp2.text, "html.parser")
    table = soup.find("table", {"class": "hasBorder"})
    if not table:
        # try any table
        table = soup.find("table")
    if not table:
        print("  No table found. Preview:", resp2.text[:300])
        return []

    results = []
    rows = table.find_all("tr")[1:]
    print("  Rows found:", len(rows))

    for row in rows:
        cells = row.find_all("td")
        if len(cells) < 4:
            continue
        date_str = cells[0].get_text(strip=True)
        code     = cells[1].get_text(strip=True)
        name     = cells[2].get_text(strip=True)
        subject  = cells[3].get_text(strip=True)

        link_tag = cells[3].find("a")
        href = ""
        if link_tag and link_tag.get("href"):
            h = link_tag["href"]
            href = h if h.startswith("http") else BASE_URL + h

        results.append({
            "\u80a1\u7968\u4ee3\u78bc": code,
            "\u516c\u53f8\u540d\u7a31": name,
            "\u5e02\u5834\u5225": market_label,
            "\u516c\u544a\u65e5\u671f": date_str,
            "\u516c\u544a\u4e3b\u65e8": subject,
            "\u516c\u544a\u9023\u7d50": href,
        })

    print("  Records:", len(results))
    return results

def main():
    year = get_roc_year()
    print("ROC Year:", year)

    records = []
    for market in ["sii", "otc"]:
        records += search_market(market, year)
        time.sleep(2)

    records.sort(key=lambda r: r.get("\u516c\u544a\u65e5\u671f", ""), reverse=True)

    output = {
        "year": year,
        "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M UTC+8"),
        "keyword": KEYWORD,
        "total": len(records),
        "records": records,
    }

    with open("data.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print("Done! Records:", len(records), "-> data.json")

main()
